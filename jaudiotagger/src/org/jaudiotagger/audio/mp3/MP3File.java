...
public class MP3File extends AudioFile
{
    ...
    public MP3File(HttpFile file, int loadOptions) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        this(file, loadOptions, false);
    }
    ...
    private void readV1Tag(HttpFile file, int loadOptions) throws IOException
    {
        if ((loadOptions & LOAD_IDV1TAG) != 0)
        {
            logger.finer("Attempting to read id3v1tags");
            try
            {
                id3v1tag = new ID3v11Tag(file, file.getName());
            }
            catch (TagNotFoundException ex)
            {
                logger.config("No ids3v11 tag found");
            ...
            try
            {
                if (id3v1tag == null)
                {
                    id3v1tag = new ID3v1Tag(file, file.getName());
                }
            }
            catch (TagNotFoundException ex)
            {
    ...
    private void readV2Tag(HttpFile file, int loadOptions) throws IOException, TagException
    {
        //We know where the Actual Audio starts so load all the file from start to that point into
        //a buffer then we can read the IDv2 information without needing any more file I/O
        int startByte = (int) ((MP3AudioHeader) audioHeader).getMp3StartByte();
    ...
    private MP3AudioHeader checkAudioStart(long startByte, MP3AudioHeader currentHeader) throws IOException, InvalidAudioFrameException
    {
        ...
        logger.warning(ErrorMessage.MP3_ID3TAG_LENGTH_INCORRECT.getMsg("", Hex.asHex(startByte), Hex.asHex(currentHeader.getMp3StartByte())));
        ...
        if(file != null)
        	...
        	newAudioHeader = new MP3AudioHeader(httpfile, 0);
        ...
        if (currentHeader.getMp3StartByte() == newAudioHeader.getMp3StartByte())
        {
            ...
            //problem
            logger.config(ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg("", Hex.asHex(newAudioHeader.getMp3StartByte())));
            return currentHeader;
        }
        else
        {
            //We get a different value if read from start, can't guarantee 100% correct lets do some more checks
            logger.config((ErrorMessage.MP3_RECALCULATED_POSSIBLE_START_OF_MP3_AUDIO.getMsg("", Hex.asHex(newAudioHeader.getMp3StartByte()))));
            ...
            if (currentHeader.getNumberOfFrames() != newAudioHeader.getNumberOfFrames())
            {
            	...
            	if(file != null)
            		nextAudioHeader = new MP3AudioHeader(file, newAudioHeader.getMp3StartByte() + newAudioHeader.mp3FrameHeader.getFrameLength());
               		...
               		nextAudioHeader = new MP3AudioHeader(httpfile, newAudioHeader.getMp3StartByte() + newAudioHeader.mp3FrameHeader.getFrameLength());
                ...
                {
                    logger.warning((ErrorMessage.MP3_START_OF_AUDIO_CONFIRMED.getMsg("", Hex.asHex(currentHeader.getMp3StartByte()))));
                    return currentHeader;
                }
                ...
                {
                    logger.warning((ErrorMessage.MP3_RECALCULATED_START_OF_MP3_AUDIO.getMsg("", Hex.asHex(newAudioHeader.getMp3StartByte()))));
    ...
    public MP3File(File file, int loadOptions, boolean readOnly) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        ...
        try
        {
            ...
            if(id3v1tag == null || (id3v1tag.getTitle().isEmpty() && id3v1tag.getArtist().isEmpty())){
            	loadOptions = loadOptions | MP3File.LOAD_IDV2TAG;
            	//Read v2 tags (if any)
    ...
    public MP3File(HttpFile file, int loadOptions, boolean readOnly) throws IOException, TagException, ReadOnlyFileException, InvalidAudioFrameException
    {
        HttpFile newFile = null;
        try
        {
            this.httpfile = file;
            //newFile = file;
